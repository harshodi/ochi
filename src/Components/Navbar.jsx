...
const Navbar = ()=>{
  return (
    <div data-scroll data-scroll-speed=".3" className='backdrop-blur z-[999] w-full px-20 py-8 font-[Neue Montreal] flex justify-between items-cente'>
        <div className='logo'>
        ...
        {/* <div className='links flex gap-10'>
            {["Service","Our work","About us","Insights","Contact us"].map((item,index)=>(
                <a key={index} className={`text-md font-regular ${index === 4 && "ml-32"}`}>{item}</a>
            ))}
        </div> */}
  ...
  {["Service", "Our work", "About us", "Insights", "Contact us"].map((item, index) => (
    <a 
      key={index} 
      className={`text-md font-regular hover:underline transition-all duration-200 ${index === 4 ? "ml-32" : ""}`}
    >
      {item}
    </a>
  ))}
...
<div className='links flex gap-10'>
  {["Service", "Our work", "About us", "Insights", "Contact us"].map((item, index) => (
    <a
      key={index}
      className={`relative group text-md font-regular overflow-hidden ${
        index === 4 ? "ml-32" : ""
      }`}
    >
      {item}
      <span className="absolute left-0 bottom-0 h-[1px] w-0 bg-gray-800 transition-all duration-300 group-hover:w-full "></span>
    </a>
  ))}
</div>
